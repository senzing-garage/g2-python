...
# <-ExampleQ-> # Set up message queue
# <-ExampleQ-> if args.queueName != '':
# <-ExampleQ->     credential_params = pika.PlainCredentials(username=args.queueUser, password=args.queuePassword)
# <-ExampleQ->     connection = pika.BlockingConnection(pika.ConnectionParameters(host=args.queueHost, credentials=credential_params))
# <-ExampleQ->     channel = connection.channel()
...
# <-ExampleQ->  if row_is_json and dsrcAction == 'X':
# <-ExampleQ->    if args.queueName == '':
# <-ExampleQ->      g2Engine_.reevaluateRecord(dataSource, recordID, 0)
# <-ExampleQ->    else:
# <-ExampleQ->      g2Engine_.reevaluateRecordWithInfo(dataSource, recordID, 0, response)
# <-ExampleQ->  else:
# <-ExampleQ->    if args.queueName == '':
# <-ExampleQ->      g2Engine_.process(row)
# <-ExampleQ->    else:
# <-ExampleQ->      g2Engine_.processWithInfo(row, response)
# <-ExampleQ->  message = response.decode('utf-8')
# <-ExampleQ->  if message != '':
...
if __name__ == '__main__':
    ...
    stop_row_redo_node.add_argument('-sr', '--stopOnRecord', dest='stopOnRecord', default=0, type=int, help='stop processing after n records (for testing large files)')
...
# <-ExampleQ-> g2load_parser.add_argument('-qu', '--queueUser', default='', help='rabbitMQ user name')
# <-ExampleQ-> g2load_parser.add_argument('-qp', '--queuePassword', default='', help='rabbitMQ password')
