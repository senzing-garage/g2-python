...
def makeKeytable(fileName, tableName):
    ...
    fileMap["sourceField"] = (
        "<csvFieldName> for the data source (only required if multiple)"
    )
    ...
    if (
        "RESOLVED_ENTITY_ID" in columnNames
        and "DATA_SOURCE" in columnNames
        and "RECORD_ID" in columnNames
    ):
        ...
        if not os.path.exists(fileName + ".map"):
            print("")
            print(
                "please describe the fields for "
                + fileName
                + " as follows in a file named "
                + fileName
                + ".map"
            )
            ...
            if "sourceField" not in fileMap and "sourceValue" not in fileMap:
                print(
                    "either a sourceField or sourceValue must be specified in the file map"
                )
                return None
    ...
    with open(fileMap["fileName"], "r") as csv_file:
        ...
        for row in csv_reader:
            ...
            rowData[fileMap["recordField"]] = (
                str(rowData[fileMap["recordField"]]) + "|DS=" + str(sourceValue)
            )
            ...
            fileMap["records"][rowData[fileMap["recordField"]]] = rowData[
                fileMap["clusterField"]
            ]
...
def erCompare(fileName1, fileName2, outputRoot):
    ...
    for side2clusterID in fileMap2["clusters"]:
        ...
        for recordID in side2recordIDs:
            ...
            try:
                ...
                auditData["_side1score_"] = fileMap1["clusters"][
                    auditData["_side1clusterID_"]
                ][recordID]
        ...
        if debugOn:
            print(
                ...
                % (
                    side1recordCnt,
                    side1clusterCnt,
                    ",".join(map(str, side1clusterIDs.keys())),
                )
            )
        ...
        if side2recordCnt > 1:
            ...
            statpack["PAIRS"]["PRIOR_COUNT"] += (
                side2recordCnt * (side2recordCnt - 1)
            ) / 2
            ...
            if len(side1clusterIDs) > 1:  # --gonna be some new negatives here
                ...
                for clusterID in side1clusterIDs:
                    if (not largestSide1clusterID) or side1clusterIDs[
                        clusterID
                    ] > side1clusterIDs[largestSide1clusterID]:
                        largestSide1clusterID = clusterID
                statpack["PAIRS"]["COMMON_COUNT"] += (
                    side1clusterIDs[largestSide1clusterID]
                    * (side1clusterIDs[largestSide1clusterID] - 1)
                ) / 2
        ...
        for side1clusterID in side1clusterIDs:
            ...
            for recordID in fileMap1["clusters"][side1clusterID]:
                if recordID not in side2recordIDs:
                    ...
                    try:
                        ...
                        auditData["_side2score_"] = fileMap2["clusters"][
                            auditData["_side2clusterID_"]
                        ][recordID]
                    auditRows.append(auditData)
            ...
            if clusterNewPositiveCnt > 0:
                if debugOn:
                    print(
                        "newer cluster %s has %s more records!"
                        % (side1clusterID, clusterNewPositiveCnt)
                    )
        ...
        if "MERGE" in auditCategory:
            ...
            for clusterID in side2clusterCounts:
                if side2clusterCounts[clusterID] > side2clusterCounts[side2clusterID]:
                    ...
                    side2clusterCounts[clusterID] == side2clusterCounts[side2clusterID]
                    and clusterID < side2clusterID
                ):
            ...
            for i in range(len(auditRows)):
                if auditRows[i]["_auditStatus_"] == "same":
                    ...
                    if (
                        ...
                        and auditRows[i]["_side1score_"] not in same_side1matchKeys
                    ):
            ...
            for i in range(len(auditRows)):
                ...
                # --see if split rows are related
                    ...
                    if relKey in fileMap1["relationships"]:
                        auditRows[i]["_side1score_"] = (
                            "related on: " + fileMap1["relationships"][relKey]
                        )
                    # else:
        ...
        for auditData in auditRows:
            ...
            csvRow.append(
                auditData["_side2score_"] if "_side2score_" in auditData else ""
            )
            csvRow.append(auditData["_side1clusterID_"])
            csvRow.append(
                auditData["_side1score_"] if "_side1score_" in auditData else ""
            )
            ...
            if (
                auditData["_auditStatus_"] in ("new negative", "new positive")
                and auditData["_side1score_"]
            ):
        ...
        if (
            len(statpack["AUDIT"][auditCategory]["SUB_CATEGORY"][bestScore]["SAMPLE"])
            < 100
        ):
            statpack["AUDIT"][auditCategory]["SUB_CATEGORY"][bestScore][
                "SAMPLE"
    ...
    eps = int(
        ...
        / (
            float(
                time.time() - batchStartTime if time.time() - batchStartTime != 0 else 1
            )
        )
    )
    batchStartTime = time.time()
    print(
        " %s entities processed at %s, %s per second, complete!" % (entityCnt, now, eps)
    )
    ...
    if statpack["ENTITY"]["NEWER_COUNT"] and statpack["ENTITY"]["PRIOR_COUNT"]:
        statpack["ENTITY"]["PRECISION"] = round(
            (statpack["ENTITY"]["COMMON_COUNT"] + 0.0)
            / (statpack["ENTITY"]["NEWER_COUNT"] + 0.0),
            5,
        )
        statpack["ENTITY"]["RECALL"] = round(
            statpack["ENTITY"]["COMMON_COUNT"]
            / (statpack["ENTITY"]["PRIOR_COUNT"] + 0.0),
            5,
        )
        if (statpack["ENTITY"]["PRECISION"] + statpack["ENTITY"]["RECALL"]) != 0:
            statpack["ENTITY"]["F1-SCORE"] = round(
                ...
                * (
                    ...
                    / (
                        statpack["ENTITY"]["PRECISION"]
                        + statpack["ENTITY"]["RECALL"]
                        + 0.0
                    )
                ),
                5,
    ...
    if statpack["CLUSTERS"]["NEWER_COUNT"] and statpack["CLUSTERS"]["PRIOR_COUNT"]:
        statpack["CLUSTERS"]["PRECISION"] = round(
            (statpack["CLUSTERS"]["COMMON_COUNT"] + 0.0)
            / (statpack["CLUSTERS"]["NEWER_COUNT"] + 0.0),
            5,
        )
        statpack["CLUSTERS"]["RECALL"] = round(
            statpack["CLUSTERS"]["COMMON_COUNT"]
            / (statpack["CLUSTERS"]["PRIOR_COUNT"] + 0.0),
            5,
        )
        if (statpack["CLUSTERS"]["PRECISION"] + statpack["CLUSTERS"]["RECALL"]) != 0:
            statpack["CLUSTERS"]["F1-SCORE"] = round(
                ...
                * (
                    ...
                    / (
                        statpack["CLUSTERS"]["PRECISION"]
                        + statpack["CLUSTERS"]["RECALL"]
                        + 0.0
                    )
                ),
                5,
        ...
        if (statpack["PAIRS"]["PRECISION"] + statpack["PAIRS"]["RECALL"]) != 0:
            statpack["PAIRS"]["F1-SCORE"] = round(
                ...
                * (
                    ...
                    / (
                        statpack["PAIRS"]["PRECISION"]
                        + statpack["PAIRS"]["RECALL"]
                        + 0.0
                    )
                ),
                5,
        ...
        if (statpack["RECORDS"]["PRECISION"] + statpack["RECORDS"]["RECALL"]) != 0:
            statpack["RECORDS"]["F1-SCORE"] = round(
                ...
                * (
                    ...
                    / (
                        statpack["RECORDS"]["PRECISION"]
                        + statpack["RECORDS"]["RECALL"]
                        + 0.0
                    )
                ),
                5,
    ...
    print(
        "%s merged entities "
        % (statpack["AUDIT"]["MERGE"]["COUNT"] if "MERGE" in statpack["AUDIT"] else 0)
    )
    print(
        "%s split entities "
        % (statpack["AUDIT"]["SPLIT"]["COUNT"] if "SPLIT" in statpack["AUDIT"] else 0)
    )
    print(
        "%s split+merge entities "
        % (
            statpack["AUDIT"]["SPLIT+MERGE"]["COUNT"]
            if "SPLIT+MERGE" in statpack["AUDIT"]
            else 0
        )
    )
    print("")
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        ...
        dest="outputRoot",
        default=None,
        help="the ouputfile root name (both a .csv and a .json file will be created",
    )
    argParser.add_argument(
        "-D",
        "--debug",
